...
Two halves, per `wiki/guides/testing.md`'s split.
...
GUARDS = periods.PeriodGuards(max_opposite_sign_ratio=3.0, concept_switch_scale_max=2.0, share_basis_max_ratio=1.5)
...
def _facts(rows: list[tuple], ticker: str = "TEST", field: str = "totalRevenue", concept: str = "us-gaap:Revenues") -> pd.DataFrame:
        ...
        out.append(
            {
                ...
                "duration_type": periods.period_shape("duration", days),
            }
        )
...
def test_the_ytd9_route_wins_when_the_two_q4_routes_disagree():
    ...
    facts = _facts(
        [
            (*_Q1, 100.0),
            ("2020-04-01", "2020-06-30", 100.0),
            ("2020-07-01", "2020-09-30", 100.0),
            (*_Y6, 200.0),
            (*_Y9, 290.0),  # the nine-month total says 290, not 300
            (*_FY, 400.0),
        ]
    )
    ...
    assert q4["basis"] == periods.FY_MINUS_YTD9
...
def test_the_quarter_sum_route_is_the_fallback_and_says_so():
    ...
    quarters = periods.quarterize(facts, _spec(), GUARDS)
    q4 = quarters[quarters["period_end"] == pd.Timestamp("2020-12-31")].iloc[0]
    assert q4["basis"] == periods.FY_MINUS_QUARTERS
    assert q4["value"] == pytest.approx(100.0)
    testable = quarters[quarters["basis"].isin([periods.AS_REPORTED, periods.FY_MINUS_YTD9])]
    print(
...
def test_an_as_reported_quarter_is_never_replaced_by_a_derived_one():
    ...
    facts = _facts(
        [
            ...
            (*_Y9, 300.0),
            (*_FY, 400.0),  # arithmetic would say 100
        ]
    )
    quarters = periods.quarterize(facts, _spec(), GUARDS)
...
def test_the_97_day_fact_a_filer_labels_fy_is_not_the_annual_anchor():
    ...
    shapes = dict(zip(facts["period_days"], facts["duration_type"], strict=False))
    assert shapes[370] == periods.ANNUAL and shapes[97] == periods.QUARTERLY
    quarters = periods.quarterize(facts, _spec(), GUARDS)
...
def test_a_negative_quarter_on_a_non_negative_field_is_refused():
    ...
    **`sign: "any"`** -- an insurer's top line carries realized investment losses -- so JPM's
    ...
    quarters = periods.quarterize(facts, spec, GUARDS)
...
def test_one_loss_quarter_does_not_destroy_the_years_fourth_quarter():
    ...
    facts = _facts(
        [
            (*_Q1, -368.0),
            ("2020-04-01", "2020-06-30", 2207.0),
            ("2020-07-01", "2020-09-30", 284.0),
            (*_Y9, 2123.0),
            (*_FY, 3695.0),
        ],
        field="netIncome",
...
def test_a_concept_switch_is_allowed_when_the_two_legs_are_the_same_size():
    ...
    quarters = periods.quarterize(facts, _spec("depAmort"), GUARDS)
    q4 = quarters.iloc[0]
    assert q4["value"] == pytest.approx(100.0)
...
def test_a_weighted_average_share_count_is_differenced_in_share_days():
    ...
    facts = _facts(
        [("2020-01-01", "2020-09-30", ytd9), ("2020-01-01", "2020-12-31", fy)],
        field="dilutedShares",
        ...
        [
            {
                ...
                "period_days": 91,
                "value": v,
                "basis": periods.AS_REPORTED,
                "known_from": pd.Timestamp(e),
                "source_concept": "x",
            ...
            for q, (s, e, v) in enumerate(
                [
                    ("2019-10-01", "2019-12-31", 1_000.0),
                    ("2020-01-01", "2020-03-31", 1_000.0),
                    ("2020-04-01", "2020-06-30", 1_000.0),
                    ("2020-07-01", "2020-09-30", 7_000.0),
                ],
                start=1,
            )
        ]
    )
    ttm = periods.trailing_twelve(quarters, spec, guards=GUARDS)
    last = ttm.iloc[-1]
    assert pd.isna(last["value"])
    assert last["dc_code"] == periods.SPLIT_BASIS_MISMATCH
    print(
...
def test_a_ttm_needs_four_contiguous_quarters_and_never_a_carried_forward_annual():
    ...
    spec = _spec()
    rows = [
        ("2020-01-01", "2020-03-31", 100.0),
        ("2020-04-01", "2020-06-30", 110.0),
        ("2021-01-01", "2021-03-31", 120.0),
        ("2021-04-01", "2021-06-30", 130.0),
    ]
    quarters = pd.DataFrame(
        [
            {
                ...
                "period_start": pd.Timestamp(s),
                "period_end": pd.Timestamp(e),
                "period_days": (pd.Timestamp(e) - pd.Timestamp(s)).days,
                "value": v,
                "basis": periods.AS_REPORTED,
                "known_from": pd.Timestamp(e),
                "source_concept": "x",
    ...
    assert set(ttm["dc_code"].dropna()) == {periods.INSUFFICIENT_QUARTERS}
    print(
        f"\nSANITY: four quarters spanning 18 months with a hole -> "
        f"{int(ttm['value'].isna().sum())}/{len(ttm)} NULL with "
        f"{periods.INSUFFICIENT_QUARTERS}. No annual is carried forward to fill it."
    )
...
def test_a_four_four_five_retail_calendar_labels_its_own_quarters():
    ...
    windows = [("2025-02-02", "2025-05-24"), ("2025-05-25", "2025-08-16"), ("2025-08-17", "2025-11-08"), ("2025-11-09", "2026-01-31")]
    quarters = pd.DataFrame(
        [
            {
                "ticker": "KR",
    ...
    labelled = periods.label_fiscal_periods(quarters, year_ends)
...
def test_the_year_the_filer_is_still_inside_labels_from_its_start():
    ...
    windows = [("2025-09-28", "2025-12-27"), ("2025-12-28", "2026-03-28"), ("2026-03-29", "2026-06-27")]
    quarters = pd.DataFrame(
        [
            {
                "ticker": "AAPL",
...
def test_a_missing_annual_report_does_not_swallow_three_years_of_quarters():
    ...
    ends = periods.fiscal_year_ends(facts)
...
_REAL_TICKERS = {
    ...
    "SWKS": ("Information Technology", "Semiconductors & Semiconductor Equipment", "Semiconductors"),
...
def real_periods() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for ticker, (sector, group, sub) in _REAL_TICKERS.items():
        ...
        filings = [
            f
            for f in company.get_filings(form=["10-K", "10-Q"])
    ...
    for _, group in facts.groupby("ticker"):
        q, t, _ = periods.build_periods(group, CATALOGUE)
        quarters.append(q)
        ttm.append(t)
...
def test_apples_fiscal_2025_fourth_quarter_revenue(real_periods):
    ...
    q4 = quarters[(quarters.ticker == "AAPL") & (quarters.field == "totalRevenue") & (quarters.period_end == pd.Timestamp("2025-09-27"))]
    assert len(q4) == 1
    assert q4.iloc[0]["value"] == pytest.approx(102.466e9, rel=1e-4)
    assert q4.iloc[0]["basis"] == periods.FY_MINUS_YTD9
...
def test_skyworks_ninety_seven_day_fiscal_2020_fourth_quarter(real_periods):
    ...
    _, quarters, _ = real_periods
    q4 = quarters[(quarters.ticker == "SWKS") & (quarters.field == "totalRevenue") & (quarters.period_end == pd.Timestamp("2020-10-02"))]
    assert len(q4) == 1
...
def test_a_derived_year_foots_to_the_number_the_filer_published(real_periods):
    ...
    annual = (
        facts[(facts.duration_type == periods.ANNUAL) & facts.value.notna()]
        .sort_values("filing_date")
        .drop_duplicates(["ticker", "field", "period_start", "period_end"], keep="last")
    )
    sums = ttm[ttm.basis == periods.TTM_FOUR_QUARTERS]
    joined = sums.merge(
        annual[["ticker", "field", "period_end", "value"]].rename(columns={"value": "as_filed"}), on=["ticker", "field", "period_end"]
    )
        ...
        out.append(
            {
                ...
                "duration_type": periods.period_shape("duration", days),
                "value": value,
                "filing_date": e + pd.Timedelta(days=30),
...
def test_an_annual_tagged_against_a_q4_context_does_not_become_q4():
    ...
    q = periods.quarterize(frame, _revenue_spec(), periods.load_guards("./configs"))
...
def test_an_annual_on_a_q4_window_with_no_annual_fact_is_refused():
    ...
    refusals: list[dict] = []
    q = periods.quarterize(frame, _revenue_spec(), periods.load_guards("./configs"), refusals=refusals)
    q4 = q[q.period_end == pd.Timestamp("2020-05-31")]
    print("\n=== SANITY CHECK: D1b, an annual on a Q4 window with no annual fact ===")
    print("  the fact      : $39,068M on a 91-day window ending 2020-05-31")
    print("  nine months   : $28,629M -- SMALLER than the 'quarter', which no real Q4 is")
    ...
    assert [r["dc_code"] for r in refusals] == [periods.AMBIGUOUS_DURATION]
...
def test_d1b_leaves_a_normal_fourth_quarter_alone_when_no_annual_exists():
    ...
    assert len(q4) == 1 and q4.iloc[0].basis == periods.AS_REPORTED
    assert not refusals
...
def test_orcls_mislabelled_years_never_become_quarters(orcl_quarters):
    ...
    coded = [r for r in refusals if r["dc_code"] == periods.AMBIGUOUS_DURATION]
    ...
    as_filed = facts[
        (facts["field"] == "totalRevenue")
        & (facts["duration_type"] == periods.QUARTERLY)
...
def test_the_retry_recovers_the_annual_the_filer_tagged_under_the_other_element(orcl_quarters):
    ...
    annual = revenue[revenue["duration_type"] == periods.ANNUAL]
...
def test_d1b_keeps_a_genuine_loss_quarter_bigger_than_the_nine_months():
    ...
    refusals: list[dict] = []
    q = periods.quarterize(frame, CATALOGUE.field("netIncome"), periods.load_guards("./configs"), refusals=refusals)
