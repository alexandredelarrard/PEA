...
- [ ] `cube_part_prices` stores `close_split`, `close_total`, `volume`, `ret`, `sector_ret`.
...
- [ ] `price_frames.ALL_FIELDS` ([price_frames.py:49](src/data_aggregate/utils/common/price_frames.py#L49)):
      `("close_split", "close_total", "volume", "ret", "sector_ret")`.
- [ ] `PriceFrames` dataclass: replace the `close` field with `close_split` and `close_total`.
      **Do not keep a `close` property alias** — that would defeat D2.
- [ ] `load_price_frames`'s hard-coded construction
      ([price_frames.py:163-169](src/data_aggregate/utils/common/price_frames.py#L163)) maps each
      requested field; add both.
- [ ] `PriceFrames.skeleton()` ([:89-90](src/data_aggregate/utils/common/price_frames.py#L89))
...
**Extract / store**
- [ ] `src/data_store/schema.py:105` — done in P1

**Cube price builder**
- [ ] [step_cube_prices.py:43](src/data_aggregate/transformers/step_cube_prices.py#L43) `PRICE_COLS`
- [ ] `_pivot_fields` ([:98-100](src/data_aggregate/transformers/step_cube_prices.py#L98)) — pivot both
- [ ] `_trading_calendar`, `_on_calendar`, `_universe_frames` ([:67-71, :142-148](src/data_aggregate/transformers/step_cube_prices.py#L67))
      — the calendar and universe should key on `close_split`
- [ ] `_daily_returns` ([:128-129](src/data_aggregate/transformers/step_cube_prices.py#L128)) — feed `close_total`

**data_utils**
- [ ] `prices_long_to_multiindex`'s `fields` map ([data_utils.py:67](src/data_aggregate/utils/common/data_utils.py#L67))
      — `{"CloseSplit": "close_split", "CloseTotal": "close_total", "Open": "open", ...}`
- [ ] `daily_returns` ([:51-53](src/data_aggregate/utils/common/data_utils.py#L51)) — **signature
...
- [ ] `step_cube_momentum.py:38` — needs both (`close_split` for ATR, `close_total` for the rest)
- [ ] `step_cube_fundamentals.py:54` and its `stock_close=` at `:74, :144, :170, :194` → `close_split`
- [ ] `step_cube_extras.py:73` and `stock_close=` at `:177, :197, :219` → `close_split`
- [ ] `step_cube_target.py:58` — needs `close_split` (mcap) **and** `ret`; see 4c
- [ ] `step_cube_text.py:50` → `close_split`

**Feature builders**
- [ ] [pit.py:73-90](src/data_aggregate/utils/common/pit.py#L73-L90) `daily_market_cap` — the
...
- [ ] [dividend_features.py:84-97](src/data_aggregate/utils/fundamentals/dividend_features.py#L84)
      `close_pos` → `close_split`. `dividendsPerShare / close_split` now cancels correctly.
- [ ] [target/factors.py:81-127](src/data_aggregate/utils/target/factors.py#L81-L127)
      `build_characteristics` — `size = -log(mcap)` takes `close_split`; any return leg takes `ret`.

**Strategy layer**
- [ ] [ls_model.py:94-99](src/strategies/utils/ls_model.py#L94) `_returns` — `rets` from
      `close_total`; the returned `close` (used as `book_prices`) → `close_split`
- [ ] `step_ls.py:36, :58, :75, :118` and `step_eq_long_only.py:61, :65` — `b.close` → `close_split`
      (execution prices, not returns)

**Peers**
- [ ] [step_deduce_peers.py:74](src/data_peers/step_deduce_peers.py#L74) — `close_total`
      (correlation of returns)

**Scripts and tests**
- [ ] `scripts/diagnose_missing_dates.py`
- [ ] `tests/conftest.py:227-271` — the synthetic price fixture must produce both columns
- [ ] `tests/data_aggregate/test_price_part_roundtrip.py:78, :90` — round-trip both
- [ ] `tests/data_aggregate/aggregate_fingerprint.py:435` — this has a **hard-coded copy** of
...
- [ ] Replace with `forward_compound(stock_ret, horizon)`, which already exists two functions
...
- [ ] This also fixes an existing inconsistency: the stock leg was a simple ratio while every
      factor leg it is differenced against was log-compounded.
- [ ] **Grep for every other `close` ratio across a shift or horizon** and classify each as price
...
- [ ] `_atr` takes `close_split`. `_macd`, `_rsi`, `high_prox_252` and the seasonal shifts take
      `close_total`.
- [ ] Document the split at the top of `features.py`: which block uses which, and why.
- [ ] Audit for any feature using a **bare price level** rather than a ratio — such a feature is
...
- [ ] Regenerate the baseline **only after** 4a-4d are complete and verified, and record in the
...
- [ ] Digests that should **not** move: anything downstream of macro only.

## 4f. Route the two PIT consumers

- [ ] [institutional_features.py:141-159](src/data_aggregate/utils/extras/institutional_features.py#L141):
...
- [ ] [insider_features.py:104](src/data_aggregate/utils/extras/insider_features.py#L104) — the
      %-of-shares leg → `sharesOutstandingPit`. `insider_net_buy_to_mcap` stays on mcap.
- [ ] [superinvestor_features.py:188-198](src/data_aggregate/utils/extras/superinvestor_features.py#L188)
      — `super_value_to_mcap` / `super_flow_to_mcap` are mcap ratios; no PIT needed. Confirm there
      is no shares-count leg.

## Verification

- [ ] `rg -n '\bclose\b' src/ tests/ scripts/` returns no price-column usage (only
      `plt.close`, `file.close` and similar)
- [ ] Full test suite passes
- [ ] `daily_market_cap` for AAPL 2020-07-31 == **1.817e12** (+/-1%), matching Sharadar's
      `1,817,315,475,360`. P0 recorded $439bn.
- [ ] Label at h=30 for a high-yielder (MO, T) is materially different from the pre-change value
      and matches a hand-computed compounded total return
- [ ] ATR(14) for a dividend payer is unchanged from P0 within rounding — it was already on a
      consistent basis and must not shift
- [ ] `inst_ownership_pct` for GOOGL post-2022-07 is in a plausible 0-1 range (the missing 20:1
      split made it wrong by 20x)
- [ ] Fingerprint baseline regenerated with a documented digest diff
