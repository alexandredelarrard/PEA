...
def test_the_ratio_is_volume_weighted_not_an_average_of_daily_ratios():
    ...
    rows = [{"date": d, "ticker": "A", "total_volume": 100_000.0, "short_volume": 90_000.0}
            for d in dates[:4]]
    rows.append({"date": dates[4], "ticker": "A", "total_volume": 10_000_000.0,
                 "short_volume": 1_000_000.0})
    hist = pd.DataFrame(rows)
    F = _shortvol_fields(hist, dates, None, None, None)
    ...
    F = _shortvol_fields(hist, idx, None, None, None)
    got = F["ic_shortvol_ratio_5d"].loc[idx[5], "A"]
    ...
    print(f"  four 90%-short days on 100k + one 10%-short day on 10m: "
...
def test_publication_lag_is_one_trading_day():
    ...
    F = _shortvol_fields(hist, dates, None, None, None)
    assert {"ic_shortvol_ratio_5d", "ic_shortvol_ratio_20d", "ic_shortvol_ratio_60d",
            "ic_shortvol_ratio_z252", "ic_shortvol_acceleration"}.issubset(F)
    # heavily-shorted S0 has the highest ratio cross-sectionally
    assert F["ic_shortvol_ratio_20d"].loc[dates[300]].idxmax() == "S0"
    ...
    expected = (short.rolling(20, min_periods=10).sum()
                / total.rolling(20, min_periods=10).sum()).loc[dates[299], "S0"]
    assert np.isclose(F["ic_shortvol_ratio_20d"].loc[dates[300], "S0"], expected), "lag broken"
    assert SHORTVOL_PUB_LAG == 1
    print("\n=== SANITY CHECK: RegSHO 1-day publication lag ===")
    print(f"  S0 (85% shorted) tops ic_shortvol_ratio_20d at {dates[300].date()}; the value at "
...
def test_price_interactions_are_one_sided_and_complementary():
    ...
    close = pd.DataFrame(
        {t: 100 * np.cumprod(1 + rng.normal(0.0004, 0.02, len(dates))) for t in tickers},
        index=dates)
    F = _shortvol_fields(hist, dates, None, close, None)
    weak = F["ic_shortvol_high_x_weak_price"]
    strong = F["ic_shortvol_high_x_strong_price"]
    ...
    z = F["ic_shortvol_ratio_z252"]
    quiet = (z < 0) & z.notna()
    assert (weak.where(quiet).fillna(0) == 0).all().all()
    print("\n=== SANITY CHECK: the two short-flow price interactions ===")
    print(f"  both legs >= 0; {int(both.to_numpy().sum())} cells fire both (must be 0); a "
...
def test_ftd_absent_date_is_nan_and_absent_ticker_is_zero():
    ...
    covered = idx[:60]
    fails = pd.DataFrame([{"date": d, "ticker": "HI", "fails_quantity": 1e5}
                          for d in covered])
    volume = pd.DataFrame({t: 1e6 for t in ("HI", "LO")}, index=idx)
    F = _fails_fields(fails, idx, None, volume)
    ratio = F["ic_ftd_to_adv20"]
    ...
    print(f"  file covers {len(covered)} of {len(idx)} grid days: HI reads "
...
def test_panel_columns_match_the_emission_map():
    ...
    fund = pd.DataFrame([{"ticker": t, "as_of": "2020-01-01", "sharesOutstanding": 5e8,
                          "sharesOutstandingPit": 5e8} for t in tickers])
    ...
    for leg in ("f_ic_shortvol_ratio_20d_vs_peers", "f_ic_shortvol_turnover_20d_xs",
                "f_ic_ftd_pct_so", "f_ic_shortvol_market_coverage"):
    ...
    print(f"  {len(emitted)} legs emitted from {len(EMISSION)} declared features, exact match; "
...
def test_market_coverage_is_split_basis_free():
    ...
    split_date = idx[60]
    regsho = pd.DataFrame([{"date": d, "ticker": "GEX",
                            "total_volume": 100_000.0, "short_volume": 40_000.0} for d in idx])
    # yfinance volume: as-traded 1,000,000 a day, then back-scaled by R = 0.125 BEFORE the split
    tape = pd.DataFrame({"GEX": [1_000_000.0 * (0.125 if d < split_date else 1.0) for d in idx]},
                        index=idx)
    ...
    after = _shortvol_fields(regsho, idx, None, None, tape,
                             splits)["ic_shortvol_market_coverage"]
    ...
    print(f"  across a 1-for-8 reverse split, true coverage 10%: unrestated reads "
...
def test_coverage_above_one_is_nulled_as_a_physical_impossibility():
    ...
    regsho = pd.DataFrame([{"date": d, "ticker": t, "total_volume": v, "short_volume": v * 0.4}
                           for d in idx
                           for t, v in (("GOOD", 250_000.0), ("REUSED", 3_000_000.0))])
    tape = pd.DataFrame({"GOOD": [1_000_000.0] * len(idx),
                         "REUSED": [1_000_000.0] * len(idx)}, index=idx)
    ...
    print(f"  GOOD reads {cov.loc[last, 'GOOD']:.2f}; a ticker whose RegSHO volume is 3x the "
          f"tape reads NaN rather than 3.0. Validated.")
