...
from src.data_extract.utils.common.symbol_tenure import (
    _aggregate_zip, _parse_filing_dates, derive_symbol_tenure,
)
...
def test_filing_dates_parse_in_both_shapes_the_sec_ships():
    ...
    parsed = _parse_filing_dates(pd.Series(
        ["03-MAR-2006", "2006-03-03", "31-DEC-2019", "2019-12-31", "  17-JUL-2012  ", "",
         "not-a-date"]))
    assert list(parsed[:4]) == [pd.Timestamp("2006-03-03"), pd.Timestamp("2006-03-03"),
...
def test_overlapping_tenures_survive_as_two_rows(tmp_path):
    ...
    for row in out.itertuples():
        print(f"  ZZZ {row.issuer_cik} {row.valid_from.date()} .. "
              f"{'open' if pd.isna(row.valid_to) else row.valid_to.date()}  n={row.n_filings}")
...
def test_every_drop_reason_is_counted(tmp_path):
    ...
    path = _write_zip(tmp_path, "2015q1", [
    ...
    print(f"  read={drops['rows_read']} kept={drops['rows_kept']} "
...
def test_store_round_trip_keeps_date_semantics(sqlite_store):
    ...
    """
    frame = pd.DataFrame({
        "symbol": ["AAA", "AAA"], "issuer_cik": ["0000000111", "0000000222"],
        "valid_from": pd.to_datetime(["2020-01-05", "2020-05-14"]),
        "valid_to": pd.to_datetime(["2020-02-28", None]),
        "n_filings": [2, 1], "source": ["form345"] * 2, "evidence": ["OLDCO", "NEWCO"],
    })
...
def test_real_cache_reproduces_the_measured_reuse_cases(real_tenure):
    ...
    for symbol in ("IR", "COR", "AVGO", "WM"):
        ...
        for row in rows.itertuples():
            ...
            print(f"     {row.issuer_cik}  {row.valid_from.date()} .. {end:>10}  "
                  f"n={row.n_filings:>5d}  {row.evidence}")
...
def test_real_cache_scale_and_determinism(real_tenure):
    ...
    print(f"  rows={len(real_tenure)}  symbols={len(per_symbol)}  "
          f"multi-CIK symbols={multi} ({share:.1%})")
