...
| ~1% of `item_text` is truncated by edgartools' item splitter (filers numbering proposals "Item 1.") | The Phase-5 fabrication guard turns these into *no rows* rather than invented rows. No HTML re-fetch. But note **which** rule does that work: the length floor CANNOT, because the shortest genuine tally in the baseline is **554 chars** (TDG's 2014 and 2019 special meetings — a complete `FOR / AGAINST / ABSTAIN` block on one stock-option plan) while HWM 2019's stub is 508. A floor high enough to reject the stub destroys two correct filings, so the floor stays at 400 and the **comma-grouped-number rule** plus the per-row grounding check are what catch a truncation: a stub cut before the tables carries no share count to read. |
| XOM's **transposed** non-director vote layout (`Votes Cast For: \| 3,495,486,371 \| 96.8 %`) is covered by the prompt and by **no test** | XOM is not in the 23-ticker baseline and there is no DB access outside Phase 6, so no real `item_text` for that geometry could be captured. Every OTHER geometry the plan named was re-sourced from the 333 real baseline narratives (see PHASE-5's RESULTS table). Recorded rather than glossed: this one rests on the prompt alone. |
| Item 5.07 vote extraction has **no accuracy gate**, only a fabrication guard | Measured, not assumed: a vote table prints no independent total and the dominant error is a column permutation, which is invariant under sums. All computable checks combined give recall 0.56 / precision 0.56, with 7 of 16 known-bad filings passing clean. `nominee_sum_matches` is therefore stored as a monitor (computable on 96%, true on 91%) and never used as a filter. Prior-year say-on-pay in the proxy is not independent either — lossy ("approximately 94%" vs 93.50%) and the denominator convention varies by state of incorporation. |
...
| `board_size` disagrees with the `def14a_directors` row count on 5 of 22 filings (+1..+4) | **Not a defect — two different populations.** `board_size` is the CURRENT board (AEE and PG each state "12 director nominees" in their own words 3×); the child table is everyone the proxy DESCRIBES, including those who served during the year and have since left. PG's footnote: "Mr. Lundgren and Ms. Woertz retired from the Board effective October 14, 2025" and "Ms. Lee is not standing for reelection"; AEE's: "former director James C. Johnson, who did not stand for re-election". They appear in the DIRECTOR COMPENSATION table because they earned fees, and are absent from the bios at **any** carve budget (verified at 20k/30k/45k/70k — the recovered set plateaus). Demanding equality would ask the extraction to DROP disclosed directors. Check C5 was re-specified as a one-sided turnover window; the informative direction is a **deficit** (a truncated roster). PFE is the one case where `board_size` may itself be understated: it states no count, and all 14 of its directors do enter the bios once the window widens. |
| T's director bios are reached only by the weak content-anchor fallback (4 of 11 surnames in-block) | T's proxy prints no "Age" in its whole front half and one "Director Since"; its bio cards label the field **Tenure**. Two candidate fixes were measured and both FAILED: a `Tenure` row token pulls T's window to board-overview prose (2 of 11, worse than the fallback) while moving 6 other filings for no gain, and making `has served as` optional in `_DIRECTOR_CONTENT_RE` changes nothing because an earlier "Director Since" match already wins. Left as measured rather than guessed at further. |
