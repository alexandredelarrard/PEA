...
- `src/modelling/` — `StepModelling` → `predictions`, `cube_signal`. Also `StepTrendAssetClass` (`step_trend_asset_class.py` + `utils_trend_asset/`): a SEPARATE, price-only multi-asset **time-series-momentum (trend/CTA) sleeve** — long assets trending up / short those trending down across `trend_asset.assets` (SPY + commodities/FX from `prices`, config `configs/modelling_trend_asset.yml`), vol-scaled, `fit()` calibrates the sleeve to `sleeve_vol_target` on train, `predict()` → daily NET returns saved to `trend_asset_returns`. It's the positive-skew diversifier for the long-biased equity book (blended in the backtest). The pure trend building blocks (`daily_vol`, `combined_forecast`, long-only `trend_scale_long_only`) live in the SHARED `src/utils/trend.py` so both this sleeve and the post-processing allocation backtest use one definition without cross-importing packages.
- `src/post_processing/` — `StepBacktest`: loads the saved per-horizon ensemble + a **projected** cube read (only the models' `feature_cols`+categoricals+target, `date >= train_end` — the OOS window), scores each method, writes flat prediction files to `data/predictions/` (`backtest_member_predictions` per method + `pred_ensemble`; `backtest_blended_signal`), then constructs the book with `utils/strategies_opt.py` (dollar/beta/sector-neutral inverse-variance optimizer + turnover-aware stepping + a **vol-regime de-risking overlay** `regime_vol_scale`: exposure = clip(target/trailing-SPY-vol, floor, cap) so the book cuts risk in high-vol periods) and reports each construction's return vs SP500 (`compare_strategies`). Legacy decile-EW book kept in `utils/strategies.py` as a comparison baseline. `blend_strategies()` (config `backtest.strategy_blend`) then combines several sleeve return streams DAY-BY-DAY — SPY + the equity L/S alpha + the `trend_asset_returns` trend sleeve — via `utils/strategies_blend.py`: inverse-vol (risk-parity) MIX weights across sleeves + one portfolio-level leverage to hit `portfolio_vol_target` (point-in-time), and reports the blend vs SP500. Also `StepAllocationBacktest` (`step_alloc_backtest.py` + `utils/strategies_alloc.py`): a SEPARATE long-only multi-asset **ALLOCATION** backtest over the long-history `macro_asset_prices` table (equity / gold / energy / 10Y-bond-TR / FX + CASH as the residual/funding leg). Layered: (1) risk-parity MIX — **ERC** (Equal Risk Contribution, correlation-aware, via cyclical coordinate descent) or inverse-vol, on **EWMA or simple** covariance; (2) a long-only **TREND** overlay (`src/utils/trend.py`) that scales an asset toward cash on downtrends; (3) a **VIX-aware RISK-ON regime tilt** (lift equity/energy risk budget when crash-prob is low — equity uptrend + low vol/VIX — cut it in stress); (4) a **vol-responsive leverage ceiling** (1× stress → 2× calm) plus an EWMA portfolio-vol target. Reports return/vol/Sharpe **per asset** + per market-regime and saves portfolio-vs-SP + weight-evolution plots under `data/output/allocation/`. Config `backtest.asset_allocation`.
