"""
short_interest_features.py  (src/data_aggregate/utils/institutionals/short_interest_features.py)
---------------------------------------------------------------------------------
...
(fetch_short_interest): [date, ticker, short_volume, total_volume].
    ...
    ic_shortvol_ratio      trailing-21d mean of short_volume / total_volume
                         (high = heavily shorted -> short-interest anomaly:
                          predicts lower forward returns)
    ic_shortvol_ratio_chg  recent (5d) minus baseline (63d) ratio
...
`ic_shortvol_days_to_cover` field is added as well.

Also folds in SEC Fails-to-Deliver (`fails_history`, from fetch_fails_to_deliver):
    ic_ftd_ratio  trailing-21d fails / trailing-21d traded volume
                            (settlement stress / naked-short pressure)
    ic_ftd_chg    recent (5d) minus baseline (63d) fails ratio
...
def _fails_fields(fails_hist: pd.DataFrame, idx: pd.DatetimeIndex,
    ...
    F["ic_ftd_ratio"] = ratio.shift(_FTD_PUB_LAG)
    ...
    F["ic_ftd_chg"] = chg.replace([np.inf, -np.inf], np.nan).shift(_FTD_PUB_LAG)
    return F
...
def _short_fields(hist: pd.DataFrame, idx: pd.DatetimeIndex) -> dict:
    ...
    F["ic_shortvol_ratio"] = ratio.rolling(21, min_periods=5).mean().shift(1)
    F["ic_shortvol_ratio_chg"] = (ratio.rolling(5, min_periods=3).mean()
    ...
    if {"short_interest", "avg_daily_volume"}.issubset(hist.columns):
        ...
        F["ic_shortvol_days_to_cover"] = dtc.replace([np.inf, -np.inf], np.nan)
    return F
