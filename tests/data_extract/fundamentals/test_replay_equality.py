...
which is a known-truth question, not an economic one (see wiki/guides/testing.md's parsing/
...
def test_harness_detects_a_missing_reason_code(tmp_path):
    ...
    harness.snapshot({_TICKER: TickerHistory(history, codes.iloc[[0]].reset_index(drop=True))}, after_dir)
