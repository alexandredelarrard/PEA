...
- [ ] Drop `"split_basis"` from all four entries: `sharesOutstanding` (`sharesbas`),
...
- [ ] Add `sharesOutstandingPit`: `{"kind": "direct", "from": "sharesbas",
      "split_basis": "count", "pit": true}` — or whatever key the emitter uses to mark "de-adjust
      into a new column rather than in place".
- [ ] **The config is hand-formatted.** A `json.dumps` round-trip reformats all 545 lines
...
- [ ] Update the `_SPLIT_ADJUSTMENT` rationale block
...
- [ ] `deadjust_splits` ([524-582](src/data_extract/utils/fundamentals_sharadar/field_map.py#L524-L582)):
...
- [ ] It must consume the P2 unioned event list, not `sharadar_actions` alone.
- [ ] Preserve the documented ordering (TTM aggregate first, then de-adjust — the reasoning is at
      [field_map.py:546-557](src/data_extract/utils/fundamentals_sharadar/field_map.py#L546-L557)).

### 3. Schema

- [ ] Add `sharesOutstandingPit DOUBLE PRECISION` to `fundamentals_history`. Hand-splice
      `sql/schema.sql`; purely additive.
- [ ] `epsDiluted` is derived *after* de-adjustment as `netIncome / dilutedShares`, so it follows
...
- [ ] Full `fundamentals_history` rebuild. Per the `thorough-rebuild-over-incremental` memory, a
      full rebuild is the right call here rather than an in-place migration.

## Verification

- [ ] `SELECT count(*) FROM fundamentals_history h JOIN fundamentals_sharadar s USING (...)
      WHERE h."sharesOutstanding" IS DISTINCT FROM s.sharesbas` == **0** on all 51,255 rows
- [ ] `sharesOutstandingPit` is non-null wherever `sharesOutstanding` is
...
- [ ] `shares_growth` shows **no** ±300% step in AAPL 2020Q3, AMZN 2022Q2, GOOGL 2022Q3,
      NVDA 2024Q2 — the split quarters. This is the free-fix check.
- [ ] `optionOverhang` is numerically unchanged from P0 (split-invariant on both bases)
- [ ] `epsDiluted` moves onto the vendor basis and reconciles against Sharadar's `epsdil`
