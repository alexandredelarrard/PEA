...
def accuracy_curve(results): 

    target = "TARGET_STOCK_+5D_SECTOR_MARKET"

    # accuracy + -
    results["PRED_+"] = 1*(results[target] > 0) 
    results["IS_+"] = 1*(results["TARGET_STOCK_+5D"] > 0) 
...
def to_predict(stocks, configs_general):

    target = configs_general["stock_modelling"]["TARGET"]
    features_list = configs_general["stock_modelling"]["FEATURES"]
    df_to_predict = stocks.loc[stocks[target].abs() > 100] 

    full_data = pd.DataFrame()
    
    for key, sectors in map_sectors.items():
        stocks_sector = df_to_predict.loc[df_to_predict["SECTOR"].isin(sectors)]
        del stocks_sector["Country"]
        stocks_sector.columns = [x.upper() for x in stocks_sector.columns]
        model = joblib.load(configs_general["resources"]["base_path"] / pl(f"data/results/model_stocks/{key}.pkl"))
        
        stocks_sector["PREDICTION"] = model.predict(stocks_sector[features_list],
                                    categorical_feature=configs_general["stock_modelling"]["categorical_features"])
        results_sector = stocks_sector[["PREDICTION", "DATE", "COMPANY", "SECTOR", "DISTANCE_STOCK_0D_TO_1W_MEAN", "DISTANCE_STOCK_0D_TO_4W_MEAN"]]
...
if __name__ == "__main__":
    ...
    stocks, missing = stock_analysis(configs_general, full_commos)
    stocks = add_commodities(stocks)
    stocks = add_external_data(stocks, dict_macroeconomy_data)
    stocks = encode_features(stocks)
    stocks = neutral_market_sector(stocks)

    stocks = stocks.loc[(stocks["Date"] < "2022-04-22")&(stocks["TARGET_STOCK_+5D"].between(-1,1))]

    # modelling
    if mode == "train":
        results_sector, model_sector  = modelling_price(stocks, configs_general)
        shap_values, valid_x = model_analysis(results_sector, configs_general)
        # save_models(model_sector, configs_general)

    # prediction_next week
    if mode == "test":
        results_sector = to_predict(stocks, configs_general)
        high = results_sector.loc[(results_sector["PREDICTION"] > 0.04)]
        high = high.sort_values("PREDICTION")[-60:]
        low = results_sector.loc[(results_sector["PREDICTION"] < -0.04)]
        low = low.sort_values("PREDICTION")[:60]
