...
| `fundamentals_check_status` | `cluster_id` | a human's decision about one `(ticker, field)` defect. `status` carries **`wontfix` and nothing else** -- `open` and `settled` are DERIVED from the ledger, because a stored `settled` that says so while the check still fires is a suppression list. `note` must carry a QUANTIFIED cost (the CLI refuses a note with no numeral). `findings_at_decision` records how big the cluster was when it was actually assessed, and the cluster **reopens automatically** if it grows past that |
