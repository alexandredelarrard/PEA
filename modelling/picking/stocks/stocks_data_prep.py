...
def distance_to_mean(df, commo, k_days = 5, distance_time=5, week=1):
    ...
    cols = [f"{commo}_{i}D" for i in range(k_days, k_days + distance_time +1)]
...
def kpis_past(df, var, commo, k_days=5):
    ...
    # for i in range(k_days, k_days + 61):
    #     df[f"DELTA_{commo}_{i}D"] = (df[f"{commo}_{i}D"] - df[f"{commo}_{i+1}D"]) / df[f"{commo}_{i+1}D"]
