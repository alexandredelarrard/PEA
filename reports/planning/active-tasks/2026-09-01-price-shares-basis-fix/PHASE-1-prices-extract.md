...
- [x] `_download_price_chunk`: fix the annotation typo `actions: False` → `actions: bool = False`,
      and add `auto_adjust: bool` as an **explicit, required** argument (no default — a default is
      how this trap gets re-set later).
- [x] Pass `auto_adjust=False` and `actions=True` from the equity path so the response carries
      `Close`, `Adj Close`, `Dividends` and `Stock Splits` in one call.
- [x] `download_ohlcv`: thread `auto_adjust` through; keep the current signature order so the
      macro call site is a one-line change.
- [x] `_normalize_prices` / `_chunk_response_to_frames`: map `Close` → `close_split` and
...
- [x] `fetch_price_history`: add `full: bool = False`. When true, bypass `resume_since` and use
...
- [x] Split-triggered re-pull (D5): before computing `since`, read `prices_splits` (P2) for events
...
- [x] `_fetch_price_leg`: pass `auto_adjust=True` explicitly, with a comment naming `SPY` as
      `equity_tr` and stating that the macro legs are consumed as RETURNS only. This keeps the
      macro output byte-identical.
- [x] It reads `raw[["date","ticker","close"]]` — under `auto_adjust=True` the normaliser must
...
- [x] `sql/schema.sql`: replace `"close"` in the `prices` block with `"close_split"` and
...
- [x] `src/data_store/schema.py:105` — `prices` Table. If it needs a `read_columns`, list both.

### 4. CLI

- [x] [cli.py:99-107](src/data_extract/cli.py#L99-L107): add `-F/--full` to `price-history`,
      matching the eight sibling commands that already have it.

### 5. The one-off re-download

- [x] Run `python -m src data_extract price-history --full` over the full 491-ticker universe.
- [x] **Write to a staging table and swap on success.** A partial failure that leaves half the
...
- [x] `trim_prelisting_bars` still runs before the upsert — a full pull carries the synthetic
      pre-listing prefix that an incremental tail does not.

## Verification

- [x] `close_split` for AAPL 2020-07-31 == **106.26** (not 102.80), and equals Sharadar `price`
- [x] `close_split` for KO 2004-02-27 == **24.98**; `close_total` == **12.79**
- [x] AMZN (non-payer) `close_split == close_total` on every row — the clean identity check
- [x] `close_total / close_split` is monotone non-decreasing in date for every payer, and → 1.0
      on the last row
- [x] MNST 2026-07-15 → 2026-08-15: no 97/47 alternation; a single clean split step
- [x] Spike-and-revert scan post-2020 returns **0** (P0 recorded 3)
- [x] Row count is within a few hundred of 3,263,505 (delistings and calendar edges aside)
- [x] **`prices_macro.equity_tr` digest matches P0's `macro_equity_tr_digest` exactly.** If it
      moved, the macro leg flipped basis — stop and fix before P4.
- [x] `SELECT count(*) FROM prices WHERE close_total IS NULL AND close_split IS NOT NULL` == 0
- [x] `-F/--full` appears in `price-history --help`
...
Keep the pre-swap `prices` table as `prices_pre_basis_fix` until P5 passes. Revert is a rename
plus reverting the schema splice.

Nothing downstream depends on the old table — the cube is empty — so the only reason to keep it
is to re-run P0's script against the old basis if a measurement needs re-checking. Drop it once
P5 is green.
