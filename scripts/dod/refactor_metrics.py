...
Gates
    ...
    G5  doc-sync: a `src/` change with no `AGENTS.md`/`CLAUDE.md`/`wiki/*.md` change
...
DOC_SYNC_SURFACE = ("AGENTS.md", "CLAUDE.md", "wiki/")
...
def loc_profile(source: str) -> dict:
    ...
    comment = sum(1 for i, ln in enumerate(lines, 1) if ln.strip().startswith("#") and i not in doc)
    return {"total": total, "code": total - blank - comment - doc_n, "docstring": doc_n, "comment": comment, "blank": blank}
...
def duplication(sources: dict[str, str], *, window: int = SHINGLE_LINES) -> dict:
    ...
    a verdict -- `wiki/guides/coding-standards.md` documents intentional duplication."""
    seen: Counter[str] = Counter()
    where: dict[str, list[str]] = {}
    for path, src in sources.items():
        body = [_normalise(ln) for ln in src.splitlines() if ln.strip() and not ln.strip().startswith("#")]
...
def baseline_source(path: str, sha: str, root: Path) -> str | None:
    ...
    if not sha or sha == "unknown":
        return None
    try:
        out = subprocess.run(["git", "show", f"{sha}:{path}"], cwd=root, capture_output=True, text=True, timeout=20, check=False)
...
def _run_pytest(targets: list[str], root: Path) -> tuple[bool, str, str]:
    ...
    py = sys.executable
    try:
        out = subprocess.run([py, "-m", "pytest", *targets, "-q", "-s"], cwd=root, capture_output=True, text=True, timeout=1800, check=False)
...
def build_gates(touched: list[str], tests: list[str], root: Path, sha: str, per_file: dict[str, dict]) -> tuple[list[Gate], dict]:
    gates: list[Gate] = []
    evidence: dict = {}
    ...
    if tests:
        ...
        gates.append(
            Gate(
                "G1",
                f"targeted tests green ({len(tests)} file(s))",
                ok and sanity,
        ...
        gates.append(Gate("G1", "targeted tests green", None, "no test file named (--tests) and none inferred from the touched paths"))
    ...
    if touched_store:
        ...
        gates.append(Gate("G2", "store boundary test green", ok, f"{len(touched_store)} data_store file(s) touched; {summary}"))
    else:
        gates.append(Gate("G2", "store boundary test green", None, "no `src/data_store/` file touched"))
    ...
    gates.append(Gate("G3", "no new `print(` under src/", added == 0, ", ".join(offenders) if offenders else "none added"))
    ...
    if removed:
        ...
        for item in removed:
            ...
            try:
                hit = subprocess.run(
                    ["git", "grep", "-l", "-w", name, "--", "src", "tests", "scripts", "app"],
                    cwd=root,
                    capture_output=True,
                    text=True,
                    timeout=30,
        ...
        gates.append(Gate("G4", "public API stable or call sites updated", True, "no public name removed"))
    ...
    shrunk = [
        f"{p} {prof['docstring_before']}->{prof['docstring_after']}"
        for p, prof in per_file.items()
        if prof.get("docstring_before") is not None and prof["docstring_after"] < prof["docstring_before"]
    ]
    ...
    gates.append(Gate("G7", f"AGENTS.md <= {AGENTS_MD_MAX_LINES} lines", 0 <= n <= AGENTS_MD_MAX_LINES, f"{n} lines"))
...
def main(argv: list[str] | None = None) -> int:
    ...
    for p in py_touched:
        ...
        after_prof = loc_profile(now) if now else {"total": 0, "code": 0, "docstring": 0, "comment": 0, "blank": 0}
    ...
    metrics_rows = [
        {
            "file": p,
            "status": d["status"],
            "loc_before": d["loc_before"],
            "loc_after": d["loc_after"],
            "code": d["code_after"],
            "docstring": d["docstring_after"],
            "comment": d["comment_after"],
    ...
    totals = {
        ...
        "docstring_lines_before_total": sum(d["docstring_before"] or 0 for d in per_file.values()),
    }
    ...
    slim = {
        p: {
            ...
            "code": d["code_after"],
            "comment": d["comment_after"],
            "docstring_before": d["docstring_before"],
            "docstring_after": d["docstring_after"],
    ...
    payload = {
        "scope": {"baseline_sha": sha, "touched": touched, "tests": tests},
