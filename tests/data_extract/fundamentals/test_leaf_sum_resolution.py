...
def test_disjoint_leaves_are_summed_when_no_total_is_reported():
    ...
    graph = ArcGraph(
        _arcs(
            [
                ("PaymentsToAcquireOilAndGasPropertyAndEquipment", "us-gaap", _INVEST, -1.0, _CF_ROLE),
                ("PaymentsToAcquireOtherPropertyPlantAndEquipment", "us-gaap", _INVEST, -1.0, _CF_ROLE),
                ("PaymentsToAcquireBusinessesNetOfCashAcquired", "us-gaap", _INVEST, -1.0, _CF_ROLE),
            ]
        )
    )
    available = frozenset(
    ...
    resolution = resolve_field(CATALOGUE.field("capex"), graph, available, CATALOGUE, "energy")

    assert resolution.method == STATEMENT_LEAF_SUM
    picked = [c for c, _ in resolution.children]
    assert picked == ["PaymentsToAcquireOilAndGasPropertyAndEquipment", "PaymentsToAcquireOtherPropertyPlantAndEquipment"]
...
def test_only_the_first_reported_alternative_in_a_group_is_taken():
    ...
    available = frozenset({"Depreciation", "AmortizationOfIntangibleAssets", "OtherDepreciationAndAmortization"})
...
def test_a_note_level_leaf_is_refused_because_of_its_role():
    ...
    resolution = resolve_field(CATALOGUE.field("depAmort"), graph, frozenset({"Depreciation"}), CATALOGUE)
...
def test_the_declared_weight_sign_excludes_a_contra_in_the_same_node():
    ...
    graph = ArcGraph(
        _arcs(
            [
                ("Depreciation", "us-gaap", _OPERATE, 1.0, _CF_ROLE),
                ("AccretionAmortizationOfDiscountsAndPremiumsInvestments", "us-gaap", _OPERATE, -1.0, _CF_ROLE),
            ]
        )
    )
    available = frozenset({"Depreciation", "AccretionAmortizationOfDiscountsAndPremiumsInvestments"})
...
def test_the_declared_weight_is_never_used_as_a_multiplier():
    ...
    resolution = resolve_field(CATALOGUE.field("capex"), graph, frozenset({"PaymentsToAcquirePropertyPlantAndEquipment"}), CATALOGUE, "utility")
...
def test_an_unclassifiable_extension_sibling_refuses_the_sum():
    ...
    available = frozenset({"PaymentsToAcquirePropertyPlantAndEquipment", "SomethingCapexLike"})

    resolution = resolve_field(CATALOGUE.field("capex"), graph, available, CATALOGUE, "utility")
...
def test_a_refusal_with_nothing_else_to_fall_back_on_is_reason_coded():
    ...
    resolution = resolve_field(CATALOGUE.field("capex"), graph, available, CATALOGUE, "real_estate")
...
def test_a_registered_extension_leaf_completes_the_sum():
    ...
    available = frozenset({"PlantAndEquipmentExpendituresUtility", "PlantAndEquipmentExpendituresNonUtility", "ConsolidationOfVIES"})

    resolution = resolve_field(CATALOGUE.field("capex"), graph, available, CATALOGUE, "utility", ticker="DTE")
...
def test_the_same_graph_without_the_register_refuses():
    ...
    available = frozenset({"PlantAndEquipmentExpendituresUtility", "PaymentsToAcquirePropertyPlantAndEquipment"})

    with_register = resolve_field(CATALOGUE.field("capex"), graph, available, CATALOGUE, "utility", ticker="DTE")
    without = resolve_field(CATALOGUE.field("capex"), graph, available, CATALOGUE, "utility", ticker="NOT-IN-THE-REGISTER")
...
def test_a_declared_but_unreported_extension_does_not_refuse():
    ...
    resolution = resolve_field(
        CATALOGUE.field("capex"), graph, frozenset({"PlantAndEquipmentExpendituresUtility"}), CATALOGUE, "utility", ticker="DTE"
    )
...
def test_insurer_capex_absence_is_structural_but_capex_stays_resolvable():
    ...
    tags_nothing = ArcGraph(
        _arcs(
            [
                ("PaymentsToAcquireAvailableForSaleSecuritiesDebt", "us-gaap", _INVEST, -1.0, _CF_ROLE),
            ]
        )
    )

    pgr = resolve_field(
        CATALOGUE.field("capex"), tags_it, frozenset({"PaymentsToAcquirePropertyPlantAndEquipment"}), CATALOGUE, "insurer", ticker="PGR"
    )
    met = resolve_field(
        CATALOGUE.field("capex"), tags_nothing, frozenset({"PaymentsToAcquireAvailableForSaleSecuritiesDebt"}), CATALOGUE, "insurer", ticker="MET"
    )
...
def test_bank_capex_is_still_short_circuited_not_merely_absence_coded():
    ...
    resolution = resolve_field(CATALOGUE.field("capex"), graph, frozenset({"PaymentsToAcquirePropertyPlantAndEquipment"}), CATALOGUE, "bank")
...
#: ⚠ EVERY ROW PINS ITS FISCAL PERIOD, and dropping that field is what broke this test.
#: The fixture takes the LATEST 10-K, so "the expected figure" silently meant "whatever the
#: newest annual column happens to hold" -- and a 10-K carries three of them. CSCO filed its
#: FY2026 10-K on 2026-09-02 and the pinned 2,811e6 moved from newest to middle column: the
#: extraction still resolved all three years correctly (FY2024 2,507M, FY2025 2,811M, FY2026
#: 2,540M, all via `statement_leaf_sum`) and the test failed anyway. AAPL was days from the
#: same fate. Pinning the period is what makes the assertion mean what it says.
_GROUND_TRUTH = [
    ...
    ("DTE", ("Utilities", "Utilities", "Multi-Utilities"), "capex", 4_429e6, "2025-12-31"),
    ("EOG", ("Energy", "Energy", "Oil & Gas Exploration & Production"), "capex", 6_594e6, "2025-12-31"),
    ("MAA", ("Real Estate", "Equity Real Estate Investment Trusts (REITs)", "Multi-Family Residential REITs"), "capex", 765.7e6, "2025-12-31"),
    ("MSFT", ("Information Technology", "Software & Services", "Systems Software"), "depAmort", 38_534e6, "2026-06-30"),
    ("GOOGL", ("Communication Services", "Media & Entertainment", "Interactive Media & Services"), "depAmort", 21_136e6, "2025-12-31"),
    ...
    (
        "AAPL",
        ("Information Technology", "Technology Hardware & Equipment", "Technology Hardware, Storage & Peripherals"),
        "depAmort",
        11_698e6,
        "2025-09-27",
    ),
    ("VLO", ("Energy", "Energy", "Oil & Gas Refining & Marketing"), "depAmort", 3_158e6, "2025-12-31"),
    ...
    ("CSCO", ("Information Technology", "Technology Hardware & Equipment", "Communications Equipment"), "depAmort", 2_811e6, "2025-07-26"),
]
...
def test_the_latest_10k_reproduces_the_filers_own_figure(latest_rows, ticker, gics, field, expected, period):
    ...
    blk = rows[(rows.field == field) & rows.value.notna() & (rows.duration_type == "annual")]
    assert not blk.empty, f"{ticker} {field}: no annual value at all"
    blk = blk.assign(period_end=pd.to_datetime(blk.period_end)).sort_values("period_end")
    want = blk[blk.period_end == pd.Timestamp(period)]
    if want.empty:
        # The pinned year has aged out of the filing's three annual columns. That is a
        # RE-PIN, not a failure: skip saying which periods the filing now carries, so the
        # next person updates the expectation against a figure they can still read.
        pytest.skip(
...
def test_aapl_and_vlo_keep_the_aggregate_and_never_the_note_leaf(latest_rows):
    ...
    $11,698M and $3,158M."""
    for ticker, concept in (("AAPL", "us-gaap:DepreciationDepletionAndAmortization"), ("VLO", "us-gaap:DepreciationAmortizationAndAccretionNet")):
        rows = latest_rows[ticker]
...
def test_dte_does_not_store_its_subsidiarys_capex(latest_rows):
    ...
    assert children == {"PlantAndEquipmentExpendituresUtility", "PlantAndEquipmentExpendituresNonUtility"}
    ...
    graph = ArcGraph(
        _arcs(
            [
                ...
                ("Depreciation", "us-gaap", "CostsAndExpenses", 1.0, "http://x/role/ConsolidatedStatementsOfOperations"),
                ("Depreciation", "us-gaap", _OPERATE, 1.0, _CF_ROLE),
            ]
        )
    )
