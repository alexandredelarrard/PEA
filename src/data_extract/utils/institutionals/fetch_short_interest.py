...
def _parse_regsho(text: str) -> pd.DataFrame:
    ...
    return out.groupby(["date", "ticker"], as_index=False)[["short_volume", "total_volume"]].sum()


def _fetch_day(day: pd.Timestamp) -> str | None:
    """Network call for one date, isolated for mocking. None if no file."""
    r = requests.get(_URL.format(yyyymmdd=day.strftime(DATE_FORMAT_COMPACT)), headers=_HEADERS, timeout=30)
...
def fetch_short_interest(context: Context, tickers: list[str], years_history: int = 15, pause: float = 0.05, full: bool = False) -> None:
    ...
    df_short = pd.concat(frames, ignore_index=True) if frames else pd.DataFrame(columns=["date", "ticker", "short_volume", "total_volume"])

    # upsert the freshly-downloaded days; the DB merges on (ticker, date)
    context.store.save(Tables.short_interest, df_short)
    logger.info(f"Saved {len(df_short)} new short-volume rows to DB table " f"'{Tables.short_interest}'")
