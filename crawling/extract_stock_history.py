...
def extract_stocks(stocks, to_extract, splits = 5):
    ...
    if splits > 0:
        ...
        extract = yf.download(to_extract, '2010-1-1')
    ...
    for i in range(splits):
        try:
            if i != splits:
                extracts[i] = yf.download(to_extract[i*size_splits:(i+1)*size_splits], '2010-1-1')
            else:
                extracts[i] = yf.download(to_extract[i*size_splits:], '2010-1-1')
        except Exception:
            pass
...
def extract_commodities(sub_loc):
    ...
    mapping_index = {"SB=F" : "sugar", "OJ=F" : "orange juice", "CT=F" : "cotton",
                    ...
                    "MGC=F" : "micro gold", "GC=F" : "gold", "ZT=F" : "2Y US bond", "ZF=F" : "5Y US bond",
                    "ZN=F" : "10Y US bond", "NQ=F" : "nasdaq 100", "ES=F" : "s&p500", "YM=F":"dow jones" }
