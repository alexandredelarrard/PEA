...
def _rp(name="FMR LLC", cik="", *, no_cik=False, pct=0.0, agg=0, sole=0, shared=0, torp="", citizenship="", member=None, comment=None):
    return SimpleNamespace(
        ...
        sole_voting_power=sole,
        shared_voting_power=shared,
        sole_dispositive_power=sole,
        shared_dispositive_power=shared,
        aggregate_amount=agg,
        percent_of_class=pct,
        type_of_reporting_person=torp,
...
def _filing(
    *,
    ...
    accession="0000315066-24-002743",
    issuer_cik="0001551182",
    issuer_name="Eaton Corp plc",
    cusip="",
    event_date="",
    rule=None,
    structured=False,
    ...
    obj = SimpleNamespace(
        ...
        rule_designation=rule,
        is_amendment="/A" in form,
        amendment_number=1 if "/A" in form else None,
...
def test_pre_mandate_zeros_become_nan_while_identity_survives():
    ...
    print(
        f"  6/6 numerics NaN, name+CIK kept ({row['reporting_person_name']}, "
...
def test_post_mandate_numbers_pass_through_with_rule_designation():
    rows = _filing_rows(
        _filing(
            ...
            structured=True,
            cusip="G29183103",
            event_date="03/31/2026",
            rule="Rule 13d-1(b)",
            persons=[_rp(name="Vanguard Capital Management", cik="", pct=7.48, agg=29042849, sole=3796733, torp="IA")],
    ...
    print(
        f"  pct={row['percent_of_class']} agg={row['aggregate_amount']:,} "
...
def test_cik_backfill_declines_rather_than_guessing():
    ...
    assert (
        _reporting_person_cik(_rp(name="Unrelated Fund", cik=""), 0, [("0000000111", "ALPHA CAPITAL LLC"), ("0000000222", "BETA ADVISERS LP")], 1)
        is None
    )
