...
from forecast_fl.aml.steps.train_ub_models_step import get_step_training_ub_models
...
from forecast_fl.aml.utils.variables_pipelines import (
    DATA_INPUTS,
    PIPELINE_OUTPUTS,
    UB_MODEL_TRAINING_PIPELINE_BASENAME,
    UB_MODEL_TRAINING_PIPELINE_DESCRIPTION,
    UB_MODEL_TRAINING_PIPELINE_ENDPOINT_NAME,
)
...
def create_model_training_pipeline(workspace, blob_datastore, env) -> Pipeline:
    ...
    data_ub_training_output = OutputFileDatasetConfig(
        destination=(blob_datastore, PIPELINE_OUTPUTS + "/outputs")
    )  # It passes a reference in the blob
    ...
    ub_training_step = get_step_training_ub_models(
        data_processing_output,
        data_ub_training_output,
        source_dir,
        run_config=aml_run_config,
        allow_reuse=AML_ALLOW_REUSE,
    ...
    training_ub_pipeline = Pipeline(
        workspace=workspace,
        steps=[data_loading_step, data_processing_step, ub_training_step],
        description="FL model training pipeline",
    )

    logging.info("ScriptRunConfig: \n {}".format(ub_training_step))

    # à partir de là, publier la pipeline
    # pipeline.publish & exposée via une API

    return training_ub_pipeline


def main_pipeline_model_ub(workspace, blob_datastore, env):
    ...
    published_pipeline = model_training_pipeline.publish(
        name=UB_MODEL_TRAINING_PIPELINE_BASENAME,
        description=UB_MODEL_TRAINING_PIPELINE_DESCRIPTION,
        version="1.1",
    )

    try:
        pipeline_endpoint = PipelineEndpoint.get(workspace, name=UB_MODEL_TRAINING_PIPELINE_ENDPOINT_NAME)
        ...
        PipelineEndpoint.publish(
            workspace,
            name=UB_MODEL_TRAINING_PIPELINE_ENDPOINT_NAME,
