...
from src.data_aggregate.utils.institutionals.value_basis import (
    ABSTAINED, DIVIDED, KEPT, MULTIPLIED, repair_value_basis,
)
...
def _filing(cik: str, value_per_share: float, *, shares: float = 42_212_390.0,
            ticker: str = "MSFT", rows: int = 3) -> pd.DataFrame:
    """One filing, `rows` holdings, each implying `value_per_share`."""
    return pd.DataFrame({
        "cik": cik, "period": PERIOD, "ticker": ticker,
        "shares": [shares] * rows,
...
def test_a_filing_already_in_dollars_is_untouched_to_the_last_bit():
    ...
    print(f"  implied ${CLOSE:.2f} vs close ${CLOSE:.2f} -> ratio "
...
def test_a_filing_in_thousands_is_divided_by_1000_on_every_value_leg():
    ...
    print(f"  implied ${CLOSE * 1000:,.0f} vs close ${CLOSE:.2f} -> ratio "
...
def test_a_filing_1000x_low_is_multiplied_by_1000():
    ...
    print(f"  ratio {float(register['median_ratio'].iloc[0]):.2e} -> factor 1000, "
          f"value_usd restored to {repaired['value_usd'].iloc[0]:.3e}. Validated.")


def test_a_filing_at_ratio_50_abstains_with_a_null_value_and_intact_shares():
    """50 is not a power of 1000. There is no units error that produces it, so guessing at a
    factor would invent a number -- the row is flagged and nulled, never dropped."""
    ...
    # ⚠ NaN, never 0.0 -- a zero would read as a real empty holding downstream
    ...
    print("\n=== SANITY CHECK: out-of-band filing abstains ===")
    print(f"  ratio {float(register['median_ratio'].iloc[0]):.1f} is in no band -> factor NA, "
          f"flag ABSTAINED, value_usd NaN (not 0.0), shares intact at "
...
def test_the_decision_is_per_filing_not_per_table():
    ...
    h = pd.concat([_filing("0000000001", CLOSE),
                   _filing("0000000002", CLOSE * 1000.0),
                   _filing("0000000003", CLOSE / 1000.0)], ignore_index=True)
    repaired, register = repair_value_basis(h, _close_split())
    ...
    print(f"  three filings, one table, one period: {got}. After repair all three report the "
...
def test_one_wild_row_cannot_flip_a_filing_because_the_statistic_is_the_median():
    ...
    print(f"  1 of 5 rows given a share count of 1 (implied price "
...
def test_a_quarter_end_off_the_trading_calendar_still_resolves_a_price():
    ...
    assert (repaired["value_basis_repaired"] == KEPT).all(), \
        "the as-of lookup failed and the quarter abstained"

    print("\n=== SANITY CHECK: as-of price lookup ===")
    print(f"  2021-12-31 removed from the price index; the repair reads back to 2021-12-30 and "
...
def test_no_close_split_degrades_instead_of_nulling_every_value():
    ...
    print("  close_split=None -> every row KEPT and value left as filed, register empty. "
