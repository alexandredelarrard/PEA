...
`test_impute_real_data_nondestructive`. The separate `drop_implausible_def14a` is the seam for
the opposite job (nulling present-but-known-wrong cells) and currently drops NOTHING: its one
rule, a 0.50 floor on `say_on_pay_support_pct`, was measured wrong on 14 of 14 sampled values
and deleted 61 real shareholder revolts. Callers still run drop -> impute in that order, so a
future cleared cell stays recoverable from the ticker's neighbouring years.
...
def drop_implausible_def14a(df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    highest-signal governance events in the table.

    Kept as a FUNCTION, not deleted: callers run drop -> impute in that order, `impute_def14a`
    is non-destructive by contract (it only writes where a value is NaN, and is unit-tested for
    exactly that), and folding a nulling step into it broke that invariant on 59 live cells once
    already. This is the documented seam for the next present-but-wrong cell class.
    """
