...
from src.validate.io import pull as pull_snapshot, write_result
...
OUT_ARGS = ("-o", "--out")
OUT_KWARGS = dict(required=True,
                  help="Run directory -- _cache/, _out/ and plots/ are created inside it. "
                       "Use reports/validate/<slug>; NEVER a path under src/.")
...
def _shared(fn: Callable) -> Callable:
    ...
    for decorator in (
        ...
        click.option("--cache/--no-cache", "use_cache", default=True, show_default=True,
                     help="Read the `pull` snapshot in <out>/_cache when one exists."),
...
def _run(check: Callable[..., CheckResult], table: str, out: str, use_cache: bool,
         config_path: str, tickers: str | None, **kwargs) -> None:
...
grain = _command("grain", checks.check_grain,
                 "Rows vs distinct declared-pk keys, panel edges, nulls in a key column.")
coverage = _command("coverage", checks.check_coverage,
                    "Tickers present vs the 491-name universe, each against its own prices span.")
profile = _command("profile", checks.check_profile,
                   "Per-column n/min/p01/p50/p99/max/sd/n_distinct, dead and constant legs.")
redundancy = _command("redundancy", checks.check_redundancy,
                      "Exact pairwise-complete Pearson over every pair, plus exact-equality counts.")
leakage = _command("leakage", checks.check_leakage,
                   "Horizon recession on the labels, and each feature against its source's publication clock.")
clip = _command("clip", checks.check_clip,
                "On-clip share per peer-z leg and tie mass per percentile leg.")
timeseries = _command("timeseries", checks.check_timeseries,
                      "Per (ticker, leg): jumps, holes and frozen spells over the leg's own support.")
bounds = _command("bounds", checks.check_bounds,
                  "Declared [lo, hi] per leg; abstains when the table declares none.")


@cli.command(help="Live columns vs a feature catalogue, asserted in BOTH directions.")
@_shared
@click.option("--catalogue", "catalogue_path", default=None, type=click.Path(path_type=Path),
              help="JSON mapping field -> description, or a .py exposing a dict named CATALOGUE. "
                   "Absent -> the check abstains.")
def catalogue(table: str, out: str, use_cache: bool, tickers: str | None, config_path: str,
              catalogue_path: Path | None) -> None:
    _run(checks.check_catalogue, table, out, use_cache, config_path, tickers,
