...
- `src/data_extract/` — `StepExtractAllData` super-step → 4 sub-steps: **prices** (prices+dividends, short interest, fails-to-deliver [SEC FTD → `fails_to_deliver`], 13F), **fundamentals** (SEC companyfacts history, earnings surprises → historical forward P/E, macro), **structure** (employees, DEF 14A LLM governance/executive-pay; both discover filings on demand via `edgar_fillings.list_filings`), **behavioral** (Wikipedia, Google Trends). Fetchers live under `utils/{prices,fundamentals,structure,behavioral,common}/`. Plus two **SEC bulk quarterly data sets** (entry points `fetch_insider_transactions(context, tickers)` / `fetch_financial_statements(context, tickers)`): **insider transactions** (Forms 3/4/5 → `insider_transactions` table) and **Financial Statement Data Sets** (num/sub XBRL → `pension_facts` table). Both cache their zips under `data/sec_bulk_cache/{insider_transactions,financial_statements}/` and are incremental by quarter (skip quarters already in the DB; re-parse cached zips only when the ticker universe grows, tracked via a `<table>_universe.json` sidecar). Downstream, `pension_facts` feeds the off-BS pension leverage and `insider_transactions` feeds the insider-trading cube signal (`insider_features.py` → `comp_insider`).
