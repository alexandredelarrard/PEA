...
from src.data_extract.utils.common.entity_lineage import (
    OVERLAP_JACCARD_SAME, OVERLAP_SHARED_SAME, TwoUniverseTickersOneEntity, UndecidedGreyBand,
    _Union, candidate_ciks, classify_overlap, derive_entity_lineage, entity_id_for,
    load_d19_allowlist, load_manual_lineage, score_overlap,
)
...
def _tenure(rows: list[tuple[str, str, str, str | None, int]]) -> pd.DataFrame:
    return pd.DataFrame(
        [{"symbol": s, "issuer_cik": c, "valid_from": pd.Timestamp(f),
          "valid_to": pd.Timestamp(t) if t else pd.NaT, "n_filings": n,
          "source": "form345", "evidence": ""} for s, c, f, t, n in rows])
...
def test_a_merge_joining_two_universe_tickers_is_refused():
    ...
    assert union.union("0000049826", "0000073124", source="owner_overlap[NTRS]",
                       confidence=0.0397) is False
...
def test_the_grey_band_raises_instead_of_guessing():
    ...
    tenure = _tenure([("AAA", "0000000111", "2006-01-05", "2012-01-01", 700),
                      ("AAA", "0000000222", "2013-01-05", None, 700)])
    roster = _roster([("AAA", "0000000222")])
    owners = {"0000000111": {f"o{i}" for i in range(31)},
              "0000000222": {f"o{i}" for i in range(28, 81)}}
    assert score_overlap(owners["0000000111"], owners["0000000222"])[0] == 3
    with pytest.raises(UndecidedGreyBand) as exc:
...
def test_candidate_set_is_universe_symbols_plus_roster_ciks():
    tenure = _tenure([("AAA", "0000000111", "2006-01-05", "2012-01-01", 5),
                      ("ZZZ", "0000000999", "2006-01-05", None, 5)])
...
def test_no_entity_holds_two_universe_tickers(live_lineage):
    ...
    entity = dict(zip(lineage["cik"].astype(str), lineage["entity_id"].astype(str)))

    per_entity: dict[str, list[str]] = {}
    for ticker, cik in zip(roster["ticker"].astype(str),
                           roster["cik"].astype("string").str.zfill(10)):
...
def test_the_worked_cases_resolve_as_the_plan_specifies(live_lineage):
    ...
    entity = dict(zip(lineage["cik"].astype(str), lineage["entity_id"].astype(str)))
    source = dict(zip(lineage["cik"].astype(str), lineage["source"].astype(str)))
    e = lambda c: entity.get(c, "E" + c)

    same = {"DD/DuPont E I": ("0000030554", "0001666700"),
    ...
    apart = {"WTW/Weight Watchers": ("0000105319", "0001140536"),
    ...
    for label, (pred, succ) in same.items():
        assert e(pred) == e(succ), f"{label}: {e(pred)} != {e(succ)}"
    for label, (other, home) in apart.items():
        assert e(other) != e(home), f"{label}: both resolved to {e(other)}"
    ...
    assert e("0001160497") == e("0001466258") == e("0000836102")     # TT's entity
    assert e("0001160497") != e("0001699150")                       # not today's IR
    ...
    for label, (pred, succ) in same.items():
        print(f"  SAME   {label:26s} {e(pred)} == {e(succ)}  (src {source.get(pred, '-')})")
    for label, (other, home) in apart.items():
        print(f"  APART  {label:26s} {e(other)} != {e(home)}  (src {source.get(other, 'singleton')})")
    print(f"  IR     both historic CIKs -> {e('0001160497')} (TT), today's IR -> {e('0001699150')}")
...
def test_d19_allowlist_covers_every_live_disagreement(live_lineage):
    ...
    for ticker, cik in zip(roster["ticker"].astype(str),
        ...
        if e(str(pick["issuer_cik"])) != e(str(cik)):
    ...
    print(f"  tickers {len(roster)}; agree {len(roster) - len(disagree)}; "
          f"disagree {len(disagree)}; all allow-listed with evidence")
    for ticker, cik, tenure_cik in disagree:
        print(f"    {ticker:6s} roster {cik} vs tenure {tenure_cik or 'NO TENURE':10s}"
              f" -- {allow[ticker][:78]}")
...
def test_the_register_beats_owner_overlap(live_lineage):
    ...
    register_ciks = {c for entry in load_registrants(CONFIG_DIR).values()
                     for c in entry.all_ciks()}
    ...
    print(f"  XOM 0000034088 / 0002115436 -> {entity['0000034088']} via "
          f"{source['0000034088']} (owner overlap scored them UNRELATED)")
