...
#: Ceiling on the share of TTM rows whose `dilutedShares / sharesOutstanding` sits outside
#: [0.5, 1.5] once split-straddling windows are excluded. The residual is the vendor's
#: multi-class `sharesbas` (BLK, ACGL and friends carry one class in `sharesbas` against a
#: total `shareswadil`), which this transform cannot repair -- so it is bounded rather than
#: asserted to zero. Set from the measured rate with room for one more such filer.
HYBRID_BASIS_RESIDUAL = 0.01
...
def synthetic_vendor(rows: list[dict]) -> pd.DataFrame:
    ...
    frame = pd.concat([frame, pd.DataFrame(np.nan, index=frame.index, columns=absent)], axis=1)
    frame["dimension"] = "ARQ"
    return frame[list(SHARADAR_SF1_COLUMNS)]


def four_quarters(ticker: str, values: dict[str, list[float]], *, n: int = 4, start: str = "2023-03-31") -> pd.DataFrame:
    ...
    for index, end in enumerate(ends):
        row = {
            ...
            "calendardate": end,
            "reportperiod": end,
            "date": end + pd.Timedelta(days=30),
            "fiscalperiod": f"{end.year}-Q{index + 1}",
        }
...
def test_every_history_column_is_mapped(field_map):
    ...
    kinds = pd.Series([field_map.columns[n].kind for n in HISTORY_STATEMENT_ORDER if n in field_map.columns]).value_counts().to_dict()
...
def test_every_sf1_column_is_accounted_for(field_map):
    ...
    accounted = (
        {s.source for s in field_map.direct.values()} | {s.source for s in field_map.extras.values()} | field_map.excluded | set(SHARADAR_ID_COLUMNS)
    )
...
def test_registers_refuse_an_unapproved_file(tmp_path):
    ...
    """
    for filename, loader in ((SHARADAR_ZERO_RULES_FILENAME, load_zero_rules), (SHARADAR_CORRECTIONS_FILENAME, load_corrections)):
...
def test_ebitda_is_top_down(ttm):
    ...
    gap["bottom_up"] = gap["netIncome"] + gap["incomeTaxExpense"] + gap["interestExpense"] + gap["depAmort"]
    gap["pct"] = (gap["ebitda"] - gap["bottom_up"]).abs() / gap["bottom_up"].abs().replace(0, np.nan)
    differing = gap[gap["pct"] > 0.01]
    print(f"top-down vs bottom-up on the SAME trailing twelve, {len(gap)} comparable rows:")
    print(
        f"  differ by >1% on {len(differing)} rows over {differing['ticker'].nunique()} "
        f"ticker(s); median {gap['pct'].median():.2%}, p90 {gap['pct'].quantile(0.9):.2%}, "
        f"worst {gap['pct'].max():.2%}"
    )
    print(gap.nlargest(3, "pct")[["ticker", "date", "ebitda", "bottom_up", "pct"]].to_string(index=False))
...
def test_debt_to_equity_is_not_vendor_de(context, ttm):
    ...
    vendor = context.store.load(Tables.sharadar_fundamentals, project=False, columns=["ticker", "dimension", "date", "de", "liabilities", "equity"])
...
def test_epsdiluted_is_derived_not_vendor_epsdil(vendor_arq, ttm):
    ...
    for name, numerator in (("netinccmn", "netinccmn"), ("netinc", "netinc"), ("consolinc", "consolinc")):
        vendor[f"matches_{name}"] = (vendor["epsdil"] - vendor[numerator] / vendor["shareswadil"]).abs() <= 0.005
...
def test_share_block_is_deadjusted_against_the_sec_cover_page(context, ttm, translated):
    ...
    # ⚠ SKIPPED, NOT PASSED, when the SEC replay does not cover these two. That table is
    # built one filer at a time, so a partial substrate is its normal in-progress state --
    # and an unscoped read of it would raise `TableEmptyError` here rather than report the
    # real situation. The skip message IS the finding: re-run once NVDA/WMT are replayed.
    wanted = ["NVDA", "WMT"]
    sec = context.store.load(
        Tables.fundamentals_history_sec, project=False, columns=["ticker", "as_of", "sharesOutstanding"], where={"ticker": wanted}, optional=True
    )
    if sec is None:
        have = context.store.load(Tables.fundamentals_history_sec, columns=["ticker"], optional=True)
        n = 0 if have is None else have["ticker"].nunique()
        pytest.skip(
            f"{name_of(Tables.fundamentals_history_sec)} carries none of {wanted} "
            f"({n} ticker(s) replayed) -- the cover-page de-adjustment is "
            f"UNVERIFIABLE here, not verified"
        )
    sec["as_of"] = pd.to_datetime(sec["as_of"])
    mine = frame[frame["ticker"].isin(["NVDA", "WMT"])][["ticker", "date", "sharesOutstanding"]].copy()
    mine["date"] = pd.to_datetime(mine["date"])
    joined = mine.merge(sec, left_on=["ticker", "date"], right_on=["ticker", "as_of"], suffixes=("_repo", "_sec")).dropna()
...
def test_ttm_is_four_discrete_quarters(field_map):
    ...
    """
    values = {"revenue": [100.0, 200.0, 300.0, 400.0], "assets": [10.0, 20.0, 30.0, 40.0], "shareswa": [8.0, 8.0, 12.0, 12.0]}
...
def test_a_gap_in_the_quarters_refuses_the_window(field_map):
    ...
    gapped = four_quarters("TEST", {"revenue": [100.0, 200.0, 300.0, 400.0, 500.0]}, n=5).drop(index=2).reset_index(drop=True)
...
def test_one_ticker_never_borrows_anothers_quarters(field_map):
    ...
    frame = pd.concat([four_quarters("AAA", values), four_quarters("BBB", {"revenue": [7.0, 7.0, 7.0, 7.0]})], ignore_index=True)
...
def test_amended_filing_does_not_break_the_window(field_map):
    ...
    print(
        f"whole windows         : {built_clean['totalRevenue'].notna().sum()} / "
    ...
    assert built_amended["totalRevenue"].notna().sum() == built_clean["totalRevenue"].notna().sum() == 2
...
def test_span_guard_nulls_a_spliced_window(field_map, caplog):
    ...
    frame["reportperiod"] = pd.to_datetime(frame["calendardate"]) + pd.to_timedelta([0, 60, 120, 180], unit="D")
...
def test_off_calendar_filers_have_a_ttm_line(context, field_map, actions):
    ...
    vendor = context.store.load(Tables.sharadar_fundamentals, project=True, where={"ticker": wanted})
    ...
    drift = (pd.to_datetime(vendor["calendardate"]) - pd.to_datetime(vendor["reportperiod"])).dt.days.abs().groupby(vendor["ticker"]).max()
    print("\n=== SANITY CHECK: off-calendar filers ===")
    for ticker in wanted:
        print(
            f"  {ticker:<5} whole TTM rows {whole.get(ticker, 0):>4}  "
...
def test_sum_optional_widens_cash_without_being_able_to_destroy_it(field_map):
    ...
    frame = pd.DataFrame(
        {
            ...
            "totalRevenue": [1.0] * 4,
            "grossProfit": [1.0] * 4,
            "stockholdersEquity": [1.0] * 4,
            "totalDebt": [1.0] * 4,
            "pretaxIncome": [1.0] * 4,
            "incomeTaxExpense": [1.0] * 4,
            "dilutedShares": [1.0] * 4,
            "basicShares": [1.0] * 4,
    ...
    assert field_map.derived["cash"].formula == ("cashAndEquivalents + coalesce(shortTermInvestments, 0)")
...
def test_coverage_of_the_built_frame(ttm, field_map, translated):
    ...
    # ⚠ `rows - 3 per ticker` IS AN UPPER BOUND, NOT THE EXPECTED COUNT, and asserting it as
    # an equality only held while the entitled history was contiguous. A duration column is
    # NULL for a ticker's first three quarters AND for every window that straddles a missing
    # or repeated quarter or fails `TTM_SPAN_DAYS` -- a mid-history gap, a fiscal-calendar
    # change, an unresolved amendment. Over full history that is 508 further rows, which the
    # old formula read as a transform defect. Reconciled here against the window predicate
    # restated from the frame's OWN quarter labels, so a row nulled WITHOUT cause -- or filled
    # without a whole window, which is the dangerous direction -- still fails.
    ordinals = quarter_ordinal(ttm["calendardate"])
    by_ticker = ttm["ticker"]
    contiguous = (ordinals - ordinals.groupby(by_ticker, sort=False).shift(3)) == 3
    reported = pd.to_datetime(ttm["reportperiod"], errors="coerce")
    span = (reported - reported.groupby(by_ticker, sort=False).shift(3)).dt.days
    whole = contiguous & span.between(*TTM_SPAN_DAYS)
...
def test_diluted_share_count_gaps_are_the_vendors(vendor_arq, ttm):
    ...
    """
    gaps = vendor_arq.groupby("ticker").agg(n=("shareswadil", "size"), n_dil=("shareswadil", "count"))
...
def test_a_ttm_window_straddling_a_split_stays_on_one_basis(field_map):
    ...
    """
    values = {"shareswadil": [200.0] * 8, "shareswa": [200.0] * 8, "sharesbas": [200.0] * 8, "consolinc": [10.0] * 8, "revenue": [100.0] * 8}
    frame = four_quarters("TEST", values, n=8)
    actions = pd.DataFrame([{"ticker": "TEST", "date": pd.Timestamp("2024-05-15"), "action": "split", "value": 2.0, "contraticker": "N/A"}])
    ...
    print(
        "PIT column is as-filed on both sides of the split; the three feature columns stay "
...
def test_post_split_share_counts_are_not_a_hybrid_basis(ttm, actions):
    ...
    ⚠ ASSERTED ON THE WINDOWS THAT STRADDLE A SPLIT, not on every row, because the bound is
    evidence of a SPLIT-STRADDLING window and nothing else. Over full history the unscoped
    bound fails on rows that have no split anywhere near them, and those are a different
    defect in `sharesbas`: BLK's 2009-2010 quarters ratio 2.60-2.88 while `sharadar_actions`
    records NO split for BLK ever, and ACGL's 2003-11-12 reads 2.39 with its only splits in
    2011 and 2018. Both are the vendor's multi-class share count, which this test cannot fix
    and must not be silenced by. They are reported and bounded below instead.
    """
    splits = split_events(actions)
    rows = ttm[["ticker", "date", "dilutedShares", "sharesOutstanding", "epsDiluted"]].dropna(subset=["dilutedShares", "sharesOutstanding"]).copy()
    rows["ratio"] = rows["dilutedShares"] / rows["sharesOutstanding"]
    rows["date"] = pd.to_datetime(rows["date"])

    # A row's TTM window is the year of period ends behind it, so a split inside that year is
    # what could have mixed two bases.
    split_dates = {t: pd.to_datetime(g["date"]).tolist() for t, g in splits.groupby("ticker")} if not splits.empty else {}
    straddles = pd.Series(
        [any(row.date - pd.DateOffset(years=1) <= d <= row.date for d in split_dates.get(row.ticker, ())) for row in rows.itertuples()],
        index=rows.index,
    )
    ...
    print(
        ...
        f"({out_of_band.mean():.3%}), of which straddling a split: "
        f"{int((out_of_band & straddles).sum())}"
    )
    print(rows[out_of_band].reindex(rows[out_of_band]["ratio"].sub(1.0).abs().sort_values(ascending=False).index).head(6).to_string(index=False))

    # A FACTOR OF THREE on the straddling rows, which is what the docstring's claim actually
    # is. [0.5, 1.5] is the right band for the REPORTED residual but not for an assertion:
    # 9 straddling windows sit between 1.51 and 1.66 (or 0.42) for reasons that are not a
    # basis mix, each checked against `sharadar_actions` -- OKE's 1.65 is convertible-preferred
    # dilution that holds across its whole history and is nowhere near its 2:1 of 2001-06-12;
    # V's 1.51 and WBD's 1.57 are as-converted multi-class against a single-class `sharesbas`,
    # again unrelated to their 4:1 and 2:1; and TMUS's 0.42 is the MetroPCS reverse merger
    # CO-DATED with its 0.5 reverse split, which really did double the share count mid-window.
    # The regression this guards reads 0.287 (AMZN 2.93bn against 10.19bn), so it still fails.
    span = rows.loc[straddles, "ratio"]
    assert (span > 1 / 3).all() and (span < 3).all(), (
        f"a TTM share count in a SPLIT-STRADDLING window is off its own period-end count by "
        f"more than a factor of three -- the window averaged a pre- and a post-split basis: "
        f"{rows[straddles & ((rows['ratio'] <= 1 / 3) | (rows['ratio'] >= 3))].to_string(index=False)}"
    )
    # The vendor's multi-class residual, bounded so it cannot grow unnoticed.
    assert out_of_band.mean() <= HYBRID_BASIS_RESIDUAL, (
        f"{out_of_band.mean():.3%} of rows are off their period-end count by >50%, over the "
