...
Features (per stock, per quarter; all peer-relativized downstream):
    ic_inst_holders                # of managers holding              (ownership breadth)
    ic_inst_breadth_chg            Δ holders vs prior quarter         (Chen-Hong-Stein)
    ic_inst_shares_chg             % QoQ chg in aggregate shares      (VOLUME accumulation)
    ic_inst_value_chg              % QoQ chg in aggregate value       (VALUE accumulation)
    ic_inst_new_buyers             # managers initiating              (fresh entrants)
    ic_inst_exiters                # managers fully exiting           (departures)
    ic_inst_new_buyer_ratio        new_buyers / holders               (fresh-money breadth)
    ic_inst_cluster_buying         (increasers - decreasers)/holders  (smart-money consensus)
    ic_inst_concentration          Herfindahl of value shares         (crowding / fragility)
    ic_inst_net_options_ratio      (call - put) / total invested      (options sentiment)
    ic_inst_net_options_ratio_chg  QoQ Δ in the ratio above           (sentiment shift)
    ic_inst_ownership_pct          13F shares / shares outstanding    (needs fundamentals)
    ic_inst_value_to_mcap          13F long value / market cap        (institutional WEIGHT)
    ic_inst_flow_to_mcap           net QoQ $ flow / market cap        (size-scaled accumulation)
"""
    ...
    for ticker, tdf in h.groupby("ticker"):
        ...
        for p in sorted(tdf["period"].unique()):
            ...
            rows.append({
                ...
                "ic_inst_new_buyers": new_b,
                "ic_inst_exiters": float(len(prev_ciks - cur_ciks)) if has_prev else np.nan,
                "ic_inst_breadth_chg": float(holders - len(prev_ciks)) if has_prev else np.nan,
                "ic_inst_shares_chg": (inst_shares / prev_shares - 1.0)
                                   if (has_prev and prev_shares > 0) else np.nan,
                "ic_inst_value_chg": (inst_value / prev_value - 1.0)
                                  if (has_prev and np.isfinite(prev_value) and prev_value > 0) else np.nan,
                "ic_inst_cluster_buying": ((inc - dec) / holders) if holders > 0 else np.nan,
                "ic_inst_new_buyer_ratio": (new_b / holders)
                                   if (holders > 0 and np.isfinite(new_b)) else np.nan,
                "ic_inst_net_options_ratio": opt_ratio,
                "ic_inst_net_options_ratio_chg": (opt_ratio - prev_opt)
...
def build_institutional_feature_panel(
    ...
    ic_inst_ownership_pct; `shares_out_history` + `stock_close` enable the value/market-cap
    ...
    feats = ["ic_inst_holders", "ic_inst_breadth_chg", "ic_inst_shares_chg", "ic_inst_value_chg",
             "ic_inst_new_buyers", "ic_inst_exiters", "ic_inst_cluster_buying", "ic_inst_new_buyer_ratio",
             "ic_inst_net_options_ratio", "ic_inst_net_options_ratio_chg", "ic_inst_concentration"]
