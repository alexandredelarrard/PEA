...
**run_id `10cc745356bc`** | 54 ticker(s) | tiers 1,2,3 | fields all

12743 finding(s) | 2332 cluster(s) | 55 field family(ies)

severity: critical=7  high=7767  medium=2620  info=2349
...
| `code_vocabulary` | 1 | history | 75,558 | 0 | 0 | 0.00% | 0.0% | ok |
...
| `peer_ratio` | 2 | facts | 83,426 | 1963 | 0 | 2.35% | 3.0% | ok |
...
| `annual_footing` | 3 | facts | 11,782 | 178 | 0 | 1.51% | 2.0% | ok |
...
| `holdout_q4` | 3 | facts | 11,782 | 222 | 0 | 1.88% | 2.0% | ok |
| `leaf_vs_total` | 3 | facts | 251,271 | 22 | 0 | 0.01% | 25.0% | ok |
| `q4_footing` | 3 | facts | 11,782 | 190 | 0 | 1.61% | 2.0% | ok |
...
|---|---|---|---|---|---|
| `incomeTaxExpense` | 5,504 | 961 | 53 | 53 of 54 tickers | **likely-check-or-catalogue** |
| `netIncome` | 4,470 | 753 | 53 | 53 of 54 tickers | **likely-check-or-catalogue** |
| `pretaxIncome` | 4,178 | 741 | 53 | 53 of 54 tickers | **likely-check-or-catalogue** |
| `basicShares` | 3,147 | 720 | 53 | 53 of 54 tickers | **likely-check-or-catalogue** |
| `dilutedShares` | 3,066 | 702 | 52 | 52 of 54 tickers | **likely-check-or-catalogue** |
| `operatingIncome` | 2,722 | 523 | 44 | 44 of 54 tickers | **likely-check-or-catalogue** |
| `interestExpense` | 2,554 | 498 | 53 | 53 of 54 tickers | **likely-check-or-catalogue** |
| `operatingCashFlow` | 2,074 | 451 | 54 | 54 of 54 tickers | **likely-check-or-catalogue** |
| `depAmort` | 1,477 | 439 | 54 | 54 of 54 tickers | **likely-check-or-catalogue** |
...
| `stockBasedComp` | 1,101 | 274 | 54 | 54 of 54 tickers | **likely-check-or-catalogue** |
...
| `costOfRevenue` | 734 | 154 | 43 | 43 of 54 tickers | **likely-check-or-catalogue** |
| `shortTermInvestments` | 720 | 167 | 45 | 45 of 54 tickers | **likely-check-or-catalogue** |
| `netInterestIncome` | 678 | 184 | 54 | 54 of 54 tickers | **likely-check-or-catalogue** |
...
| `sellingGeneralAdmin` | 614 | 175 | 37 | 37 of 54 tickers | **likely-check-or-catalogue** |
| `researchAndDevelopment` | 514 | 246 | 54 | 54 of 54 tickers | **likely-check-or-catalogue** |
...
| `noninterestIncome` | 354 | 86 | 8 | 8 of 54 tickers | **likely-filer** |
...
| 3 | `b09464043fc1` **<-- B's menu** | APA | `operatingIncome` | 320 | 46 | 6 | high | likely-check-or-catalogue |
| 4 | `0b47bf08db6e` **<-- B's menu** | AMT | `depAmort` | 318 | 51 | 4 | high | likely-check-or-catalogue |
| 5 | `5e4107839207` **<-- B's menu** | UNP | `ppeNet` | 318 | 63 | 3 | high | likely-check-or-catalogue |
| 6 | `2943d47a9b6b` | BA | `operatingIncome` | 315 | 44 | 7 | high | likely-check-or-catalogue |
...
| 11 | `9d0ebfbe91bb` | C | `interestExpense` | 305 | 36 | 7 | high | likely-check-or-catalogue |
...
| 25 | `d7c1802a4e98` | MSFT | `netIncome` | 235 | 29 | 7 | high | likely-check-or-catalogue |
...
- seen in 3 comparable run(s), 2026-08-24 -> 2026-08-25
...
### 3. APA `operatingIncome` -- `b09464043fc1`  **<-- B's menu**
...
### 4. AMT `depAmort` -- `0b47bf08db6e`  **<-- B's menu**
...
### 5. UNP `ppeNet` -- `5e4107839207`  **<-- B's menu**
...
- seen in 3 comparable run(s), 2026-08-24 -> 2026-08-25
- https://www.sec.gov/Archives/edgar/data/100885/000010088519000196/0000100885-19-000196-index.htm
- _the only rule that catches a value resolved to an entirely wrong concept without a human noticing first_
...
### 11. C `interestExpense` -- `9d0ebfbe91bb`

- **score 305** from 36 finding(s) across 7 check(s)
- checks agreeing: `peer_ratio`x18, `trend_break`x6, `cross_vintage`x5, `level_outlier`x4, `basis_step`x1, `catalogue_exclusion_cost`x1, `series_shape`x1
- severity: high=31, medium=4, info=1 | tier: T1=1, T2=30, T3=5
...
- _an interior gap with modal code None -- present before AND after, so the filer kept reporting and we stopped resolving. A MISSING TAG_
...
- _an interior gap with modal code 'split_basis_mismatch' -- present before AND after, so the filer kept reporting and we stopped resolving. A MISSING TAG_
...
### 25. MSFT `netIncome` -- `d7c1802a4e98`

- **score 235** from 29 finding(s) across 7 check(s)
- checks agreeing: `peer_ratio`x15, `cross_vintage`x7, `trend_break`x3, `annual_footing`x1, `holdout_q4`x1, `q4_footing`x1, `scale`x1
- severity: high=28, medium=1 | tier: T2=19, T3=10
- periods: 2010-09-30..2023-09-30
- routing: **likely-check-or-catalogue** (53 of 54 tickers on this field)
- *7 INDEPENDENT checks agree here. That is a far stronger prior than one check firing 29 times.*
- seen in 3 comparable run(s), 2026-08-24 -> 2026-08-25
