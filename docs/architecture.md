...
| `StepCubeInstitutionals` | `cube_part_institutionals` | `close_split, close_total, volume, level_factor, sector_ret, ret` | **7 panels**: broad 13F, elite 13F, insider, beneficial ownership (13D+13G), short flow (RegSHO volume + fails-to-deliver), and the two DERIVED ones — the `ic_sig_*` price-conditioning layer and `ic_xs_*` cross-source agreement. The two derived panels are functions of a `ConditioningSink` the first five fill on the way past (event dates, bullish actors, and 7 value-plus-availability signal payloads), because re-deriving the insider dates would repeat a 2M-row scope-and-repair pass and the elite dates the per-manager availability join |
...
| `StepAssembleCube` | `cube` | — | read the parts → `base LEFT JOIN` the wide targets → chunked write, one row per (date, ticker) |
