...
def _close(seed: int = 0) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        {t: 100 * np.cumprod(1 + rng.normal(0.0004, 0.015, len(IDX))) for t in TICKERS},
        index=IDX)
...
def _events(dates: dict[str, list[int]]) -> dict[str, pd.DataFrame]:
    ...
    rows = [{"ticker": t, "date": IDX[i], "value": 1e5, "shares": 100.0}
            for t, positions in dates.items() for i in positions]
...
def test_age_days_is_nan_before_the_first_event_then_counts_trading_days():
    ...
    print(f"  A: NaN for 100 days, 0 on the event, +1 per trading day to 199, resets to 0 on "
...
def test_the_panel_moves_on_every_day_with_no_filing():
    ...
    daily = ("f_ic_sig_insider_age_days", "f_ic_sig_insider_ret_since",
             "f_ic_sig_insider_resid_ret_since", "f_ic_sig_insider_price_vs_buy")
    moved = {}
    for col in daily:
        d = a[col].diff().dropna()
    ...
    print(f"  {span[0].date()} -> {span[-1].date()}, zero events: "
          + ", ".join(f"{c.removeprefix('f_ic_sig_')} moved {n}/{len(span) - 1} days"
...
def test_no_future_prices_reach_a_conditioning_value():
    ...
    print(f"  rebuilt with prices truncated at {IDX[cut - 1].date()}: max |diff| over "
...
def test_excursions_are_signed_capped_and_anchored():
    ...
    print(f"  run-up >= 0 and drawdown <= 0 always; both 0 on the event day and NaN before it; "
...
def test_the_cost_anchor_is_restated_across_a_split():
    ...
    got = fixed.loc[(fixed["ticker"] == "A") & (fixed["date"] == day),
                    "f_ic_sig_insider_price_vs_buy"].iloc[0]
    ...
    print(f"  100 shares at $1,000, then 10-for-1, now $100: unrestated reads "
...
def test_every_declared_feature_is_emitted():
    ...
    print(f"  {len(EMISSION)} features -> {len(emitted)} legs, exact match with the EMISSION "
