...
_WHY_FROZEN = ("no `daily_legs`, so there is no leg the builder promises to recompute every "
...
def _frozen_legs(spec: TableSpec, columns: list[str]) -> tuple[list[str], int | None, str]:
    ...
    if spec.daily_legs:
        ...
        return legs, spec.frozen_min_days, (
            f"the {len(legs)} leg(s) the table declares as rebuilt every session "
            f"(`daily_legs`), at >= {spec.frozen_min_days} identical days")
    if spec.cadence == "quarterly":
        ...
        return list(columns), spec.ffill_horizon_days, (
            f"every leg, at > {spec.ffill_horizon_days} identical days -- the declared ffill "
            f"horizon for this quarterly table")
    return [], None, f"ABSTAINED -- {_WHY_FROZEN}"


def _measure(series: pd.Series, dates: pd.DatetimeIndex, spec: TableSpec, *,
             frozen_limit: int | None) -> dict[str, list[dict[str, Any]]]:
    """JUMP, HOLE and FROZEN for one (ticker, leg) series, over the leg's own support."""
    found: dict[str, list[dict[str, Any]]] = {"jump": [], "hole": [], "frozen": []}
    ...
    if span > 0 and np.unique(live).size > spec.jump_min_distinct:
        ...
        for k in np.flatnonzero((score > spec.jump_z) & material):
            found["jump"].append({
                "date": stamps[k], "value": float(support[k]),
                "previous": float(support[k - 1]) if k else float("nan"),
                "change": float(change[k]), "z": round(float(score[k]), 1),
    ...
    for start, length in _runs(~np.isfinite(support)):
        if length >= spec.hole_min_days:
            found["hole"].append({"days": length, "start": stamps[start],
    ...
    if frozen_limit is not None:
        ...
        for start, length in _runs(observed[1:] == observed[:-1]):
            ...
            if flat >= frozen_limit:
                ...
                found["frozen"].append({"days": flat, "value": value,
...
def check_timeseries(context: Context, table: Table | str, *, config: DictConfig,
    ...
    if date_col is None or ticker_col is None:
        return CheckResult.abstained(
            CHECK, spec_t.name,
            f"this is a (ticker x time) question and the table declares "
            f"date_col={date_col!r}, ticker_col={ticker_col!r}")

    columns = feature_columns(context, spec_t)
    if not columns:
        return CheckResult.abstained(CHECK, spec_t.name, "no numeric leg to measure")

    names = ([t.strip().upper() for t in tickers] if tickers
             else sorted(str(t).strip().upper()
                         for t in context.store.distinct(spec_t, ticker_col)))
    if not names:
    ...
    for index, ticker in enumerate(names, start=1):
        frame = context.store.load(spec_t, columns=[date_col] + columns,
                                   where={ticker_col: ticker}, order_by=date_col,
                                   optional=True)
        ...
        for leg in columns:
            found = _measure(frame[leg], stamps, spec,
                             frozen_limit=frozen_limit if leg in frozen_set else None)
            for kind, bucket in (("jump", jumps), ("hole", holes), ("frozen", frozen)):
                for row in found[kind]:
                    bucket.append({"ticker": ticker, "leg": leg, **row})
        if index % 25 == 0 or index == len(names):
            log.info("timeseries %s: %d/%d tickers, %s jump / %s hole / %s frozen",
                     spec_t.name, index, len(names), f"{len(jumps):,}", f"{len(holes):,}",
                     f"{len(frozen):,}")

    if not measured_tickers:
        return CheckResult.abstained(CHECK, spec_t.name,
                                     f"none of the {len(names)} requested ticker(s) has a row")
    ...
    for leg, entry in sorted(by_leg_hole.items(), key=lambda kv: -kv[1]["worst"]["days"])[:_MAX_FINDINGS]:
        ...
        findings.append(Finding.at(
            6, field=leg, ticker=worst["ticker"],
    ...
    for leg, entry in sorted(by_leg_saturated.items(),
                             key=lambda kv: -kv[1]["worst"]["days"])[:_MAX_FINDINGS]:
        worst = entry["worst"]
        findings.append(Finding.at(
            3, field=leg, ticker=worst["ticker"],
    ...
    scope = {"rows": rows, "tickers": measured_tickers, "first_date": first_date,
             "last_date": last_date, "legs": len(columns),
             "series": measured_tickers * len(columns),
             "jump_z": spec.jump_z, "jump_span_frac": spec.jump_span_frac,
             "jump_min_distinct": spec.jump_min_distinct,
             "hole_min_days": spec.hole_min_days, "min_support": _MIN_SUPPORT,
             "cadence": spec.cadence, "frozen_legs": len(frozen_legs),
             "frozen_limit_days": frozen_limit, "frozen_scope": frozen_how,
             "source": "db(per ticker)"}
    metrics = {
        "n_jumps": len(jumps), "n_holes": len(holes), "n_frozen": len(stalled),
        "n_frozen_at_extreme": len(saturated),
        "legs_with_holes": sorted(by_leg_hole), "legs_with_frozen": sorted(by_leg_frozen),
        "legs_saturated": sorted(by_leg_saturated), "legs_with_jumps": sorted(by_leg_jump),
