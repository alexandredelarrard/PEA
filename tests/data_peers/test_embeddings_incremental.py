...
def _fakestore(embedded: dict[str, list[float]] | None = None):
    ...
    if not embedded:
        return FakeStore()
    return FakeStore({"ticker_embeddings": pd.DataFrame({"ticker": list(embedded), "embedding": [list(v) for v in embedded.values()]})})
...
def test_only_missing_tickers_are_embedded(monkeypatch):
    ...
    out = emb.get_openai_embeddings({"CCC": "chip maker", "DDD": "retail bank"}, store=store, universe=universe, client=_FakeOpenAI())
    ...
    print(
        f"  done={sorted(done)}, todo={todo}; OpenAI called once for {_OAI_CALLS[0]} "
...
def test_all_cached_makes_zero_openai_calls(monkeypatch):
    ...
    out = emb.get_openai_embeddings({}, store=store, universe=["AAA", "BBB"], client=_Boom())
    assert _OAI_CALLS == []
