...
| `fundamentals_history` | `ticker, as_of` | `as_of` | quarterly | **THE MERGED TABLE, and the one consumers should read** -- Sharadar owns a declared block of columns for all history, `fundamentals_history_sec` owns the rest, and no column ever switches source mid-series (field-block precedence). **DECLARED BUT NOT YET BUILT**: nothing writes it until the Sharadar-integration phase 4, so a read returns EMPTY rather than raising. It is registered early so all 12 `src/data_aggregate/` files already point at the right name and phase 4 only has to populate it. Deliberately does NOT carry `is_amendment` / `amended_fiscal_end` / `amended_fields` -- those are pure SEC reconciliation columns, they stay on `fundamentals_history_sec`, and Sharadar has no amendment events so they would be permanently null here. No `source` column either: precedence is per-COLUMN and fixed, so a per-ROW source would be a lie |
